...
export default function UploadSong() {
    ...
    const onSubmit = async (data: FormModel) => {
        ...
        const id = await uploadDetailsResponse.json();
...
function RaisedInputPlaceholder({ className, placeholder, inputClass, onchange, before, ...props }: RaisedInputPlaceholderProps) {
    ...
    return (
        <label className={`group relative cursor-text ${className}`}>
            <span
                className={`absolute my-auto origin-left cursor-text px-2 align-bottom transition-transform duration-300 group-focus-within:-translate-y-4 group-focus-within:scale-90 ${inputClass} ${value.length > 0 && "-translate-y-5 scale-90"}`}>
                {placeholder}
            </span>
